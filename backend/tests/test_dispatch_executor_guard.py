...
@pytest.mark.asyncio
async def test_claude_code_default_forwards_cli_hint_to_adapter(
    db_session, mock_tenant_id, seeded_tenant
):
    """The executor_type specialization must reach the worker as a
    per-task ``executor`` hint so the worker picks the right CLI even
    if it auto-detected a different default at startup."""
    await _make_cfg(
        db_session,
        mock_tenant_id,
        executor_type="claude_code",
    ...
    assert isinstance(adapter, WorkerDispatchAdapter)
    assert adapter._executor == "claude_code"


@pytest.mark.asyncio
async def test_worker_default_honors_executor_field_in_config(
    db_session, mock_tenant_id, seeded_tenant
):
    """A plain ``worker`` executor_type can still pin a CLI via the
    config JSON (``{"executor": "opencode"}``)."""
    await _make_cfg(
        db_session,
        mock_tenant_id,
        executor_type="worker",
    ...
    await _register_worker(
        db_session, mock_tenant_id, capabilities=["opencode", "claude_code"]
    )
    stream = MagicMock()
    stream.publish = AsyncMock()
    ...
    assert isinstance(adapter, WorkerDispatchAdapter)
    assert adapter._executor == "opencode"


@pytest.mark.asyncio
async def test_worker_default_without_cli_pin_sends_no_hint(
    db_session, mock_tenant_id, seeded_tenant
):
    """Plain ``worker`` without a CLI pin → worker uses its own startup
    default; backend sends no executor hint."""
    await _make_cfg(
        db_session,
        mock_tenant_id,
        executor_type="worker",
