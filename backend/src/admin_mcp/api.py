...
* ``ToolRegistry`` is the single dispatcher: validate input, enforce
  every entry of ``required_scopes`` against the caller's
...
import structlog
from bsvibe_authz import IntrospectionClient, Settings, User
...
async def resolve_tool_context(
    ...
    *,
    ...
    return ToolContext(user=user, app_state=app_state)
