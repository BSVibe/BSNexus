...
from backend.src.core.state_machines import can_advance_run_attempt_phase
...
async def advance_phase(
    *,
    ...
    if not can_advance_run_attempt_phase(attempt.phase, target):
        raise RunAttemptStateError(f"Cannot transition RunAttempt from {attempt.phase.value} to {target.value}")
...
async def record_tool_event(
    *,
    ...
    if event_input.tool_name not in ALLOWED_TOOLS_BY_PHASE[attempt.phase]:
        raise RunAttemptStateError(f"Tool {event_input.tool_name!r} is not allowed in {attempt.phase.value}")
    ...
    termination_reason = await _terminal_reason_after_event(attempt=attempt, event=event, session=session)
    nudge = await _record_repetition_nudge_if_needed(attempt=attempt, event=event, session=session)
    if termination_reason is not None:
        await finish_run_attempt(
            attempt=attempt,
            status=RunAttemptStatus.failed if "repeated_tool_call" in termination_reason else RunAttemptStatus.timed_out,
            terminal_reason=termination_reason,
            session=session,
        )
        await session.refresh(event)
        return ToolEventResult(event=event, terminated=True, terminal_reason=termination_reason, nudge=nudge)
...
async def _record_repetition_nudge_if_needed(
    *,
    ...
    if len(recent_events) < 2:
        return None
    if any(recent_event.tool_name != event.tool_name or recent_event.args_hash != event.args_hash for recent_event in recent_events):
        return None
