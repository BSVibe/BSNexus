...
class BSageKnowledgeClient:
    ...
    async def backlinks(self, path: str) -> list[str]:
        ...
        try:
            ...
            logger.warning("bsage_backlinks_failed", path=path, error=str(exc), reason="response_parse")
            return []
    ...
    async def record_decision(
        ...
        *,
        ...
        try:
            ...
            logger.warning("bsage_decision_failed", title=title[:60], error=str(exc), reason="response_parse")
            return None
        return KnowledgeEntryRef(
