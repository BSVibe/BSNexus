"""Host-side sandbox backend — the fallback when ``sandbox_enabled``
is false.

``NoopSandboxManager`` runs commands as host subprocesses and file ops
as host filesystem IO, rooted at the project workspace directory —
exactly the behaviour ``ToolRegistry`` and the verifier have today. It
keeps the loop unchanged until the DinD-backed manager is flipped on.
"""
...
from backend.src.core.sandbox.errors import SandboxError
...
class NoopSandboxSession:
    ...
    def _resolve(self, rel_path: str) -> Path:
        """Resolve a workspace-relative path, refusing escapes. The
        docker backend gets this confinement for free from the ``/work``
        mount; the noop backend enforces it explicitly."""
...
class NoopSandboxManager:
    """Host-side manager — every project shares the host; ``acquire``
    just hands back a session rooted at the workspace. Lifecycle calls
    are no-ops (there is no container to reap)."""
