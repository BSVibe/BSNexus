...
from backend.src.core.domain import ProofAspectStatus
...
_JUDGE_TEXT_EXTS = frozenset(
    {".py", ".md", ".txt", ".toml", ".json", ".yaml", ".yml", ".cfg", ".ini", ".js", ".ts", ".tsx", ".go", ".rs", ".sh"}
)
...
async def judge_criteria(
    *,
    ...
    if failed:
        lines = "; ".join(
            f"FAIL: {str(v.get('criterion') or '?')[:120]} — {str(v.get('reason') or '')[:200]}" for v in failed
        )
