...
async def _run_setup(sandbox_session: SandboxSession, command: str) -> tuple[bool, str | None]:
    ...
    if result.exit_code != 0:
        ...
        logger.warning("repo_setup_failed", command=command, exit_code=result.exit_code, detail=detail)
...
async def ensure_repo_dependencies(
    *,
    ...
    if devcontainer_commands:
        ...
        return InstallResult(status="installed", detail="; ".join(devcontainer_commands), source="devcontainer")
