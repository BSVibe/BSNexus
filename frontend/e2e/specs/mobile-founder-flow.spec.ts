...
test.describe('G7 mobile founder flow — Direct / Decide / Review', () => {
  ...
  test('Review — Brief renders the 5 sections in order without horizontal overflow', async ({ page }) => {
    await page.goto(`/projects/${PROJECT_ID}?tab=brief`, { waitUntil: 'networkidle' })

    // Wait for the Brief surface to mount.
    await expect(page.locator('[data-brief-surface]').first()).toBeVisible({ timeout: 10_000 })

    const headings = await page.locator('[data-brief-surface] h2').allTextContents()
